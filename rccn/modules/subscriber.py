...
from config import (db_conn, sq_hlr_path, config, api_log, roaming_log, RIAK_TIMEOUT, NoDataException, use_nitb_osmo_stack)
...
class Subscriber:
    ...
    def add(self, msisdn, name, balance, location='', equipment=''):
        if len(msisdn) == 15:
            ...
            extension = self.get_msisdn_from_imsi(msisdn)
            if len(extension) == 11:
...
if __name__ == '__main__':
    print(Subscriber().get_all_roaming())
